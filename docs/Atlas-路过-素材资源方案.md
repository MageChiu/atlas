...
容器层（地球/亚洲/中国/四川）与成都共用同一脚本与投影机制，各自有一套 SVG 模板（`BUILDERS` 注册表，按 `regionId` 分发）：

| 区域 | regionId | geoBounds (W/E/S/N) | 投影 | mapSize | 地物 |
| --- | --- | --- | --- | --- | --- |
| 地球 | region_earth | -180/180/-90/90 | 等距圆柱(Plate Carrée) | 2048×1024 | 五大洲暖色轮廓 + 海洋底 |
| 亚洲 | region_asia | 25/170/-12/82 | 线性(低精度) | 2048×1621 | 东南/西南海域 + 喜马拉雅 |
| 中国 | region_china | 73/135/18/54 | 线性(低精度) | 2048×1470 | 东南海域 + 西部高原 + 长江/黄河 |
| 四川 | region_sichuan | 97/110/26/34 | 线性(低精度) | 2048×1455 | 西部横断山地 + 成都平原 + 岷江 |

- 世界层用**等距圆柱**而非墨卡托（墨卡托纵向非线性会与线性投影错位），bbox 2:1。
- 子区域用自身 `Region.geo` 经 `geoToPixel` 投影到**父**底图定位（如"中国"落在亚洲图偏东、"成都"落在四川图中部）。
- 各层 PNG 实际像素与 seed `mapSize` 完全一致（SVG 即按 mapSize 生成），`assertBoundsRatio` 全部 OK（偏差 ≤0.1%），无需回填。
- 青城山-都江堰世界遗产路线为**主题分组**，只有 `geo` 无 `geoBounds`/`mapSize`，不出独立底图——它作为节点投影到四川图上，进入后直接出景点。
