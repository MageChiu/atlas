...
- [x] 字段：`sessionId / npc(NpcPublic) / messages(DialogueMessage[]) / status / sending / grantedTriggerIds`。
- [x] 方法：`startSession / appendMessage / setSending / applyGrants / reset`。
- [x] 仅存会话/交互态；不重复缓存后端实体（服务端数据由 TanStack Query/调用结果驱动）。

### TF05-3 useDialogue（`hooks/useDialogue.ts`）
- [x] 封装 `start` 与 `say`（TanStack Query `useMutation`）：
...
- [x] 达到结束态（`status=Ended`）禁用输入。

### TF05-4 对话面板 UI（`components/domain/DialoguePanel.tsx` + `DialogueMessageList.tsx`）
- [x] 展示 NPC 头像/名（`avatar` 走 `NEXT_PUBLIC_ASSET_BASE`，缺图占位兜底，复用现有 assets 解析风格）。
- [x] 消息流（user/npc 区分气泡）+ 输入框 + 发送按钮；`sending` 时禁用并显示"对方正在思考…"。
- [x] 收获到达时在消息流内插入轻提示（"获得：草堂诗卡 · 已收入图鉴"），奖励详情走 `RewardPopup`。
- [x] 关闭面板时 `dialogueStore.reset()`；不使用页面跳转。

### TF05-5 入口分发（`hooks/useExecuteAction.ts` + `stores/uiStore.ts` + `ActionPanel`）
- [x] encounter 类型改为**开对话**：点击 encounter Action → `useDialogue.start(spotId, action 关联的 npcId?)` → 打开 `DialoguePanel`（经 `uiStore.openModal({type:'dialogue', spotId, ...})`，新增一类 modal type）。
- [x] 不再对 encounter 走 `executeAction`/静态 `EncounterPanel`（保留静态 EncounterPanel 仅用于仍返回 `event` 的旧数据，按需保留或标注）。
- [x] 分发集中在统一入口，不在组件里散落 if-else（遵循前端规范第 8 节）。

### TF05-6 验证
- [x] `npm run build:web` 通过。
- [x] Mock 模式（`NEXT_PUBLIC_USE_MOCK=true`）下走通：在杜甫草堂点偶遇 → 开对话 → 多轮输入 → 输入含"春夜喜雨"触发收获 → RewardPopup + 图鉴可见。
- [x] 切真实接口（`USE_MOCK=false`）零字段改动即可联通（与后端 `tasks/05-NPC对话-后端.md` 对齐）。

---

## 3. 验收
- [x] encounter → 多轮对话面板（不跳页），NPC 公开信息与文案均来自接口、无写死。
- [x] 收获经服务端返回的 `grants` 展示并接入图鉴；前端不自行判定发奖。
- [x] 错误码（RISK_BLOCKED/LLM_UNAVAILABLE/SESSION_ENDED）均有合理 UI。
- [x] Mock 与真实接口结构一致；写操作带 requestId；`npm run build:web` 通过。
